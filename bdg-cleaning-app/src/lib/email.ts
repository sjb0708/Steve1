...
export async function sendEmail({ to, subject, html }: EmailOptions): Promise<SendResult> {
  ...
  if (gmailUser && gmailPass) {
    try {
      ...
      // sendMail resolves even when the server refuses SOME recipients, so a
      // rejected address looked exactly like a success and vanished silently.
      // Check the response instead of trusting the absence of a throw.
      if (info.rejected?.length) {
        console.error(`[email] rejected by server: ${info.rejected.join(", ")} — ${info.response}`)
        return { ok: false, detail: `Mail server rejected ${to}.` }
      }
      if (!info.accepted?.length) {
        console.error(`[email] no recipient accepted for ${to} — ${info.response}`)
        return { ok: false, detail: `Mail server did not accept ${to}.` }
      }
  ...
  if (resendKey) {
    ...
    try {
      ...
      const { error } = await resend.emails.send({ from: fromEmail, to, subject: subject || "", html })
      if (error) {
        console.error("Resend send failed:", error)
        return { ok: false, detail: error.message ?? "Resend rejected the message." }
      }
      return { ok: true, detail: "sent via Resend" }
    } catch (err) {
      console.error("Resend send failed:", err)
      return { ok: false, detail: "Resend threw an error." }
    }
  }
...
// One shared shell for admin alerts so every alert — cron reminders, new
// turnovers, test sends — looks like the rest of the app's mail instead of
// each one growing its own bespoke template.
const TONES = {
  info:   { bar: "#1e40af", sub: "#bfdbfe", panel: "#f8fafc", border: "#e2e8f0" },
  action: { bar: "#d97706", sub: "#fde68a", panel: "#fffbeb", border: "#fde68a" },
  urgent: { bar: "#dc2626", sub: "#fecaca", panel: "#fef2f2", border: "#fecaca" },
  good:   { bar: "#059669", sub: "#a7f3d0", panel: "#f0fdf4", border: "#bbf7d0" },
}
...
export function adminAlertEmail({
  ...
  const detailBlock = details.length
    ? `<div style="background: ${t.panel}; border: 1px solid ${t.border}; border-radius: 8px; padding: 16px; margin: 20px 0;">
        ${details
          .map(
            (d) =>
              `<p style="margin: 0 0 6px; color: #64748b; font-size: 14px;">${d.label}</p>
               <p style="margin: 0 0 12px; font-weight: bold; color: #0f172a;">${d.value}</p>`
          )
          .join("")}
      </div>`
    : ""
  const bulletBlock = bullets.length
    ? `<div style="background: ${t.panel}; border: 1px solid ${t.border}; border-radius: 8px; padding: 16px; margin: 20px 0;">
        ${bullets.map((b) => `<p style="margin: 0 0 10px; color: #0f172a; font-size: 15px;">• ${b}</p>`).join("")}
      </div>`
    : ""
  const ctaBlock =
    ctaLabel && ctaUrl
      ? `<a href="${ctaUrl}" style="display: inline-block; background: ${t.bar}; color: white; padding: 14px 28px; border-radius: 8px; text-decoration: none; font-weight: bold; font-size: 16px;">${ctaLabel}</a>`
      : ""
  return `
    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
      <div style="background: ${t.bar}; padding: 24px; border-radius: 12px 12px 0 0;">
        <h1 style="color: white; margin: 0; font-size: 20px;">${heading}</h1>
        <p style="color: ${t.sub}; margin: 4px 0 0;">Bailey Development Group</p>
      </div>
...
export function jobReminderEmail(
  ...
  return `
    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
      <div style="background: ${needsResponse ? "#d97706" : "#1e40af"}; padding: 24px; border-radius: 12px 12px 0 0;">
        <h1 style="color: white; margin: 0; font-size: 20px;">${needsResponse ? "Please Respond — Cleaning Coming Up" : "Upcoming Cleaning Reminder"}</h1>
        <p style="color: ${needsResponse ? "#fde68a" : "#bfdbfe"}; margin: 4px 0 0;">Bailey Development Group</p>
      </div>
      <div style="background: white; padding: 24px; border: 1px solid #e2e8f0; border-top: none; border-radius: 0 0 12px 12px;">
        <p style="color: #334155;">Hi ${cleanerName},</p>
        <p style="color: #334155;">${needsResponse
          ? "A cleaning offered to you is coming up soon and still needs your answer:"
          : "A friendly reminder about your upcoming cleaning:"}</p>
        <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 16px; margin: 20px 0;">
          <p style="margin: 0 0 8px; color: #64748b; font-size: 14px;">PROPERTY</p>
          <p style="margin: 0; font-weight: bold; color: #0f172a;">${propertyName}</p>
          <p style="margin: 8px 0 0; color: #64748b; font-size: 14px;">DATE</p>
        ...
        <a href="${actionUrl}" style="display: inline-block; background: ${needsResponse ? "#d97706" : "#1e40af"}; color: white; padding: 14px 28px; border-radius: 8px; text-decoration: none; font-weight: bold; font-size: 16px;">
          ${needsResponse ? "Accept or Decline" : "View Job Details"}
        </a>
        <p style="color: #94a3b8; font-size: 12px; margin-top: 24px;">Bailey Development Group Cleaning Management</p>
      </div>
    </div>
