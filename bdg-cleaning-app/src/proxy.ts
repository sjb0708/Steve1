...
const PUBLIC_ROUTES = ["/login", "/logout", "/forgot", "/set-password", "/privacy", "/terms"]
...
export async function proxy(request: NextRequest) {
  ...
  const isPublic =
    ...
    pathname.startsWith("/api/price-evaluation/collect") ||
    pathname.startsWith("/api/market-analysis/collect") ||
...
export const config = {
  matcher: [
    // .webmanifest and .ico must be excluded too — phones fetch the manifest
    // to build the home-screen icon, sometimes without cookies, and a redirect
    // to /login silently breaks "Add to Home Screen".
