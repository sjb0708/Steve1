...
import { supplyRequestedEmail } from "@/lib/email"
...
export async function POST(req: NextRequest) {
  try {
    ...
    await alertAdmins({
      ...
      emailSubject: `Supplies needed at ${job.property?.name}`,
      emailHtml: (admin) =>
        supplyRequestedEmail(
          admin.name,
          user.name,
          job.property?.name ?? "",
          items.join(", "),
      ...
      smsBody: `BDG Cleaning: ${user.name} requested supplies at ${job.property?.name}: ${itemList}. ${APP_URL}/jobs/${jobId}`,
