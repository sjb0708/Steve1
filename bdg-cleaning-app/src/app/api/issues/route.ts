...
import { issueReportedEmail } from "@/lib/email"
...
export async function POST(req: NextRequest) {
  try {
    ...
    await alertAdmins({
      ...
      emailSubject: `Issue reported at ${job.property?.name}`,
      emailHtml: (admin) =>
        issueReportedEmail(
          admin.name,
          user.name,
      ...
      smsBody: `BDG Cleaning: ${user.name} reported a ${severityLabel.toLowerCase()}-severity ${typeLabel.toLowerCase()} issue at ${job.property?.name}.${costSuffix} ${issueUrl}`,
