...
import {
  ...
  ArrowLeft, AlertCircle, UserCheck, FileText, ThumbsUp, ThumbsDown, MessageSquare,
  ...
  async function handleAssign() {
    ...
    try {
      const res = await fetch(`/api/jobs/${jobId}`, {
        ...
        body: JSON.stringify({ cleanerId: selectedId, silent: silent || undefined }),
      })
      if (!res.ok) {
      ...
      <div className="p-6 space-y-4">
        ...
        <label className="flex items-start gap-2.5 p-3 bg-slate-50 border border-slate-200 rounded-xl cursor-pointer">
          ...
          <span className="text-sm text-slate-700">
            Don&apos;t notify the cleaner — assigns quietly as already accepted. No email, text, or accept request.
          </span>
        </label>
        {error && (
...
function AdminJobDetail({ job: initialJob }: { job: Job }) {
  ...
  const [textingCleaner, setTextingCleaner] = useState(false)
  const [textResult, setTextResult] = useState<{ ok: boolean; name: string; detail: string[] } | null>(null)

  // Manual send. Goes out even while automated cleaner notifications are
  // paused — the pause is for automation, this is a deliberate button press.
  async function sendClosingText() {
    setTextingCleaner(true)
    setTextResult(null)
    try {
      const res = await fetch(`/api/jobs/${job.id}/text-cleaner`, { method: "POST" })
      const d = await res.json().catch(() => ({}))
      if (!res.ok) {
        setTextResult({ ok: false, name: "", detail: [d.error ?? "Could not send."] })
      } else {
        const channels: string[] = []
        if (d.smsSent) channels.push("Text delivered.")
        if (d.emailSent) channels.push("Email sent.")
        setTextResult({
          ok: d.smsSent || d.emailSent,
          name: d.sentTo,
          detail: [...channels, ...(d.problems ?? [])],
        })
      }
    } catch {
      setTextResult({ ok: false, name: "", detail: ["Network error. Try again."] })
    } finally {
      setTextingCleaner(false)
    }
  }
                    ...
                    {job.status !== "COMPLETED" && (
                      <div className="space-y-2">
                        <Button
                          ...
                          size="sm"
                          className="w-full"
                          onClick={sendClosingText}
                          disabled={textingCleaner}
                        >
                          {textingCleaner ? <Spinner size="sm" /> : <><MessageSquare className="w-4 h-4" /> Text the Before-You-Leave List</>}
                        </Button>
                        {textResult && (
                          <div className="text-xs rounded-xl p-3 bg-slate-50 border border-slate-200 space-y-1">
                            <p className={textResult.ok ? "text-emerald-700 font-medium" : "text-amber-700 font-medium"}>
                              {textResult.ok ? `Sent to ${textResult.name}` : "Not sent"}
                            </p>
                            {textResult.detail.map((d) => (
