...
import {
  UserCheck, UserX, Users, Mail, Phone, MapPin, User as UserIcon,
  Plus, Copy, CheckCircle2, AlertCircle, Shield, Pencil, KeyRound, Trash2,
} from "lucide-react"
...
function AddCleanerModal({ open, onClose, onAdded }: AddCleanerModalProps) {
  ...
  return (
    <Modal
      ...
      onClose={() => { reset(); onClose() }}
      title="Add Cleaner"
      description="Only a name is needed to put them on the schedule — add email/phone whenever you have it"
    >
      <div className="p-6">
        <form onSubmit={handleSubmit} className="space-y-4">
          <Input
...
function EditCleanerModal({ user, onClose, onSaved }: EditCleanerModalProps) {
  const [form, setForm] = useState({
    name: "", username: "", email: "", phone: "", carrier: "", paymentMethod: "", paymentDetails: "",
  })
  const [submitting, setSubmitting] = useState(false)
  const [error, setError] = useState("")
    ...
    <Modal open={!!user} onClose={onClose} title="Edit Cleaner" description={user?.name}>
      <div className="p-6">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Input label="Username (for logging in)" value={form.username} icon={UserIcon}
            placeholder="e.g. tperez"
            autoCapitalize="none" autoCorrect="off" spellCheck={false}
            onChange={(e) => setForm((f) => ({ ...f, username: e.target.value }))} />
          <p className="text-xs text-slate-500 -mt-2">
            This is what they type to sign in. Leave blank if they don&apos;t need app access.
          </p>

          <Input label="Email (optional)" type="email" value={form.email} icon={Mail}
          ...
          <p className="text-xs text-slate-400 -mt-2">
            {form.carrier
              ? "Job offers and updates go out by text" + (form.email ? " and email together." : ". Add an email to also send email.")
              : form.email
              ? "No carrier set — job offers and updates go out by email only. Pick a carrier above to also send free texts."
              : "No email or carrier — this cleaner can't receive job offers yet. Add an email, or a phone + carrier for texts."}
          </p>
...
function UserCard({ user, pending, onApprove, onReject, onDeactivate, onDelete, onEdit, onSendInvite, actionLoading }: UserCardProps) {
          ...
          <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap gap-x-4 gap-y-1 pt-1 mt-1 border-t border-slate-50 text-xs text-slate-500">
                  <span>
                    Notify: <span className="font-medium text-slate-700">
                      {user.email && user.carrier ? "Email + Text"
                        : user.email ? "Email only"
                        : user.phone && user.carrier ? "Text only"
                        : "Not reachable"}
                    </span>
                    {user.email && !user.carrier && (
                      <span className="text-amber-600 font-medium"> (add a carrier to enable free texts)</span>
                    )}
                    {!user.email && !(user.phone && user.carrier) && (
                      <span className="text-amber-600 font-medium"> (add an email, or a phone + carrier)</span>
                    )}
                  </span>
                  <span>
            ...
            {(pending || onDeactivate || onEdit) && (
              <div className="flex flex-wrap gap-2 mt-3">
                ...
                {!pending && user.role === "CLEANER" && onSendInvite && !!user.email && (
                  <Button
                    size="sm"
                    variant="outline"
                ...
                {!pending && onDelete && user.role === "CLEANER" && (
                  <Button
                    size="sm"
                    variant="outline"
                    className="text-red-600 border-red-200 hover:bg-red-50"
...
export default function UsersPage() {
  ...
  async function handleDelete(target: User) {
    if (!confirm(`Delete ${target.name} permanently? Their open jobs go back to Unassigned. This can't be undone.`)) return
    ...
    try {
      ...
      if (res.ok) {
        ...
        const d = await res.json().catch(() => ({}))
        alert(d.error ?? "Failed to delete cleaner.")
      }
    } finally {
      setActionLoading(null)
    }
