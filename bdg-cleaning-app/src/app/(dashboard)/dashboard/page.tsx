...
interface DashStats {
  ...
  unassignedLater: number
  horizonLabel: string
  horizonIncludesNextMonth: boolean
  assignedJobs: number
...
function AdminDashboard() {
    ...
    async function load() {
      try {
        const [statsRes, jobsRes] = await Promise.all([
          ...
          fetch("/api/jobs?limit=5&upcoming=1"),
        ])
  ...
  const statCards = [
    ...
    {
      // Scoped to the same window the alerts use, so this tile is a to-do
      // list for now — not a running total of every future booking.
      title: `Need Cleaner${stats?.horizonLabel ? ` — through ${stats.horizonLabel}` : ""}`,
      value: stats?.unassignedJobs?.toString() ?? "—",
      icon: AlertCircle,
      iconColor: stats?.unassignedJobs ? "text-amber-600" : "text-emerald-600",
      iconBg: stats?.unassignedJobs ? "bg-amber-50" : "bg-emerald-50",
      change: stats?.unassignedJobs
        ? "Assign a cleaner"
        : stats?.unassignedLater
          ? `All covered · ${stats.unassignedLater} in later months`
          : "All covered",
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-amber-900">
                    {stats.unassignedJobs} cleaning{stats.unassignedJobs !== 1 ? "s" : ""} need a cleaner
                    {stats.horizonLabel ? ` through ${stats.horizonLabel}` : ""}
                  </p>
                  <p className="text-xs text-amber-700 mt-1">Assign a cleaner to keep your schedule on track.</p>
                  <Link href="/jobs?status=UNASSIGNED">
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-slate-900">
                    {stats.horizonLabel} is fully covered
                  </p>
                  <p className="text-xs text-slate-500 mt-1">
                    {stats.unassignedLater} cleaning{stats.unassignedLater !== 1 ? "s" : ""} further out
                    {stats.horizonIncludesNextMonth ? "" : " — you'll get a reminder two weeks before the month ends"}.
                  </p>
                  <Link href="/jobs?status=UNASSIGNED">
                    <Button size="sm" variant="outline" className="mt-3">See Later Months</Button>
                  </Link>
                </div>
              </div>
            </Card>
...
function CleanerDashboard() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const res = await fetch("/api/jobs?limit=10&upcoming=1")
        if (res.ok) {
